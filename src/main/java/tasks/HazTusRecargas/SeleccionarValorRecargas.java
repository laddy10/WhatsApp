...
public class SeleccionarValorRecargas implements Task {
    ...
    public SeleccionarValorRecargas(User addCredentials) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        String valorBuscado = normalizarTexto(addCredentials.getValor());
        ...
        while (!paqueteEncontrado && intentos < MAX_INTENTOS) {
            ...
            for (WebElementFacade opcion : opciones) {
                ...
                if (texto.contains(valorBuscado)) {
                    actor.attemptsTo(
                            ClickTextoQueContengaX.elTextoContiene(addCredentials.getValor()),
                    ...
                    actor.attemptsTo(
                            ...
                            ValidarTextoQueContengaX.elTextoContiene(addCredentials.getValor()));
            ...
            if (!paqueteEncontrado) {
                ...
                if (!masOpciones.isEmpty()) {
                    ...
                    throw new RuntimeException("No se encontró el valor y no hay más opciones disponibles: " + addCredentials.getValor());
                }
            }
        }

        if (!paqueteEncontrado) {
            throw new RuntimeException("No se encontró el valor tras " + MAX_INTENTOS + " intentos: " + addCredentials.getValor());
        }
    }
    ...
    public static Performable seleccionarValorRecargas(User addCredentials) {
        return instrumented(SeleccionarValorRecargas.class, addCredentials);
    }
}