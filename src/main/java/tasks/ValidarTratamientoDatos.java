  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    if (!linktratamientodatos.isEmpty()) {
      ...
      CapturaDePantallaMovil.tomarCapturaPantalla(
          "Seleccionar linea de consulta y Validar politica de tratamientos de datos");
      ...
      if (!lblvistaprevia.isEmpty()) {
        actor.attemptsTo(Atras.irAtras(), WaitForResponse.withText(URL_PORTAL_CLARO));
      }
      ...
      if (!btnsi.isEmpty()) {
        actor.attemptsTo(Click.on(BTN_SI2));

      } else if (!btnsiautorizo.isEmpty()) {
        actor.attemptsTo(Click.on(BTN_SI_AUTORIZO));

      } else {
        actor.attemptsTo(Click.on(BTN_SI));
      }

      actor.attemptsTo(WaitForResponse.withAnyText(VER_MENU_PREPAGO, MENU_PRINCIPAL));
    }
  }
