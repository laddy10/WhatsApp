    ...
    for (String[] tipoDoc : tiposDocumento) {
      try {
        ...
        ReportHooks.registrarPaso(
            "ERROR: No se pudo seleccionar " + tipoDoc[1] + " - " + e.getMessage());
      }
    }

    // Volver a seleccionar C.C. por defecto para continuar el flujo
    actor.attemptsTo(
        Click.on(SELECT_TIPO_DOCUMENTO), ClickTextoQueContengaX.elTextoContiene(CEDULA_CIUDADANIA));

    CapturaDePantallaMovil.tomarCapturaPantalla(
        "Validación completa - Cédula de Ciudadanía seleccionada");
