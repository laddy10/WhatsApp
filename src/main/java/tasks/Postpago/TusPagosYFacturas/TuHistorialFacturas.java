...
public class TuHistorialFacturas implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    actor.attemptsTo(ValidarTextoQueContengaX.elTextoContiene(PAGA_FACTURA_AQUI));
  ...
  private void validarUltimas6Facturas(Actor actor) {
    ...
    for (WebElementFacade mensaje : mensajes) {
      ...
      if (texto.contains("Valor: $") || texto.contains("6") || texto.contains("https://yoiz.me/")) {
        facturasEncontradas++;
      }
    }
    ...
    if (facturasEncontradas >= 6) {
      ReportHooks.registrarPaso(
          "✅ Se encontraron " + facturasEncontradas + " facturas en el historial");
    } else {
      ReportHooks.registrarPaso(
          "⚠️ Solo se encontraron " + facturasEncontradas + " facturas (se esperaban mínimo 6)");
    }
  }
